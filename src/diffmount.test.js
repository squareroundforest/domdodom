/* global document beforeEach test expect */

import {captureDOM, capturedEq, initDOM, root} from "./domtest"
...
test("no diff", function() {
  const test = tag.div(
    ...
    tag.ul(tag.li("foo"), tag.li("bar"), tag.li("baz"))
  )
...
test("only required diff", function() {
  ...
  const update = tag.div(
    {id: "test"},
    tag.h1({className: "title"}, "A list:"),
    tag.ul(tag.li("foo"), tag.li("qux"), tag.li("baz"))
  )
