...
export const getChanges = (eq, from, to) => {
  const changes = []
...
export const syncChanges = (insert, remove, from, to, changes) => {
  let offset = 0
  for (let c of changes) {
    if (c[2] !== c[3]) {
      to = remove(to, c[2] + offset, c[3] + offset)
      offset += c[2] - c[3]
    }

    if (c[0] !== c[1]) {
      to = insert(to, c[3] + offset, from.slice(c[0], c[1]))
      offset += c[1] - c[0]
    }
  }
...
export const forEachUnchanged = (from, to, changes, proc) => {
  let fromStart = 0
  let toStart = 0
  for (let c of changes) {
