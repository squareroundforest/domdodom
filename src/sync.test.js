...
import {forEachUnchanged, getChanges, sync, syncChanges} from "./sync"

function insert(a, at, items) {
	a.splice(at, 0, ...items)
	return a
}

function remove(a, start, end) {
	a.splice(start, end - start)
	return a
}

function testGetChanges(title, from, to, expected) {
	test(`change set: ${title}`, () =>
		expect(getChanges(Object.is, from, to)).toEqual(
			expected
		))
}

function testSyncChanges(title, from, next) {
	test(`apply: ${title}`, function() {
		const c = getChanges(Object.is, from, next)
		expect(
			syncChanges(insert, remove, from, next, c)
		).toEqual(next)
	})
}
...
testGetChanges(
	...
	[1, 2, 3],
	[-1, 0, 1, 1.3, 1.6, 2, 3, 4, 5],
	[[0, 0, 0, 2], [1, 1, 3, 5], [3, 3, 7, 9]]
)
testGetChanges(
	"remove start, prepend",
	[-0.5, 1, 2, 3],
	[-1, 0, 1, 2, 3],
	[[0, 1, 0, 2]]
)
...
testGetChanges(
	"remove middle, prepend",
	[0, 1, 2, 3],
	[1, 1.3, 1.6, 2, 3],
	[[0, 1, 0, 0], [2, 2, 1, 3]]
)
testGetChanges(
	"remove middle, insert",
	[1, 2, 2.5, 3],
	[1, 1.3, 1.6, 2, 3],
	[[1, 1, 1, 3], [2, 3, 4, 4]]
)
testGetChanges(
	"remove middle, insert, same",
	[1, 1.5, 2, 3],
	[1, 1.3, 1.6, 2, 3],
	[[1, 2, 1, 3]]
)
testGetChanges(
	"remove middle, append",
	[1, 2, 3, 4],
	[1, 1.3, 1.6, 2, 3],
	[[1, 1, 1, 3], [3, 4, 5, 5]]
)
...
test("for each unchanged", function() {
	const items = []
	forEachUnchanged(
		[-1, 0, 1, 3, 5, 6, 7],
		[1, 2, 3, 4, 5],
...
test("for each unchanged, with tail", function() {
	const items = []
	forEachUnchanged(
		[-1, 0, 1, 3, 5],
		[1, 2, 3, 4, 5],
