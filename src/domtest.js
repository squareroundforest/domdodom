...
export const initDOM = n => {
  document.body.innerHTML = markup(n({id: 'root'}))
  root = document.getElementById('root')
}

const capturedElementsAndText = captured => captured.filter(c =>
  [Node.ELEMENT_NODE, Node.TEXT_NODE].some(t => t === c.node.nodeType)
)

export const captureDOM = node => {
  const captured = {node: node}
...
export const capturedEq = (prev, next) => {
